...
{	$email=$_POST["email"];
	...
	if($response->success){
	if($p==$pass){
	$sql = "Insert INTO member(email,password,name)VALUES('$email','$passmd5','$name')";
	if (mysqli_query($connect, $sql)) {
		$_SESSION["user"] = $row["name"];
			echo "Creact account successfully";
		} 
		mysqli_close($conn);
	header("Location:index.php"); }
	else{?><script>window.alert("Please make sure the confirm password & password are match and completed the reCAPTCHA!!!");window.location.href="index.php?err=signup"; </script><?php}}
	else{?><script>window.alert("Please complete the reCAPTCHA !!!");window.location.href="index.php?err=signup"; </script> <?php }}} ?> 
