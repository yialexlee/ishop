...
`		<h2 class="title text-center mb-2">Order Details</h2><!-- End .title text-center mb-2 -->
        <div style="padding-left:200px;padding-right:200px;padding-bottom:80px;padding-top:30px;">
		<?php
if(isset($_GET["oid"]))
{
	$oid = $_GET["oid"];
	$sql = "Select * from ((orderdetail inner join product on orderdetail.Product_ID = product.Product_ID)inner join member on orderdetail.Member_ID = member.id) where orderdetail.Order_ID = '$oid'";
...
?>
		<form action="#" method="POST">
										<div class="form-group">
                                            ...
                                            <input type="text" class="form-control"  name="total" value="<?php echo $row["Order_AMOUNT"] ?>" required>
                                        </div>
										<div class="form-group">
                                            <label ><strong>Recipient Name</strong></label>
                                            <input type="text" class="form-control"  name="name" value="<?php echo $row["Order_RECIPIENT"] ?>" required>
                                        </div>
										<div class="form-group">
                                            <label ><strong>Recipient Address</strong></label>
                                            <input type="text" class="form-control"  name="address" value="<?php echo $row["Order_ADDRESS"] ?>" required>
                                        </div>
										<div class="form-group">
                                            <label ><strong>Recipient State</strong></label>
                                            <input type="text" class="form-control"  name="state" value="<?php echo $row["Order_STATE"] ?>" required>
                                        </div>
										<div class="form-group">
                                            <label ><strong>Recipient City</strong></label>
                                            <input type="text" class="form-control"  name="city" value="<?php echo $row["Order_CITY"] ?>" required>
                                        </div>
										<div class="form-group">
                                            <label ><strong>Recipient Postcode</strong></label>
                                            <input type="text" class="form-control"  name="postcode" value="<?php echo $row["Order_POSTCODE"] ?>" required>
                                        </div>
										<div class="form-group">
                                            <label ><strong>Order Status </strong></label><br>
											<?php $selected = $row['Order_STATUS'];?>
                                            <select name="status" style="width:130px;height:38px;border-radius:6px;">
											<option <?php if($selected == "Paid"){echo("selected");}?> value="Paid">Paid</option>
											<option <?php if($selected == "Confirmed"){echo("selected");}?> value="Confirmed">Confirmed</option>
											<option <?php if($selected == "Shipped"){echo("selected");}?> value="Shipped">Shipped</option>
											</select>
                                        </div>
...
$status=$_POST["status"];
$sql= "Update orderdetail set Order_AMOUNT='$total',Order_RECIPIENT='$name',Order_PHONE='$phone',Order_ADDRESS='$address',Order_STATE='$state',Order_CITY='$city',Order_POSTCODE='$postcode',Order_STATUS='$status' where Order_ID='$oid' ";
if(mysqli_query($connect,$sql)){mysqli_close($connect);echo "<script> window.location.assign('orderlist.php'); </script>";}}?>
